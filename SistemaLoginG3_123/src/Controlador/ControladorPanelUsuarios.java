...
public class ControladorPanelUsuarios {
    ...
    //Asigna la funcionalidad a todos los componentes
    ...
    // Busca un usuario por ID y lo muestra (solo) en la tabla
    private void buscar() {
        ...
        try {
            ...
            DefaultTableModel tablaModelo = modelo.listarUnUsuario(id);

            if (tablaModelo.getRowCount() > 0) {
                this.vista.tablaUsuario.setModel(tablaModelo);
                limpiarCampos(); // Limpia el formulario
            } else {
                JOptionPane.showMessageDialog(this.vista, "Usuario no encontrado.", "Búsqueda fallida", JOptionPane.INFORMATION_MESSAGE);
                limpiarYRecargar();
            }
    ...
    // Limpia todo y enfoca el cursor en el ID
    private void salir() {
        limpiarCampos();
        this.vista.tablaUsuario.setModel(new DefaultTableModel());
        this.vista.TxtUsuario.requestFocusInWindow();
    }
}
